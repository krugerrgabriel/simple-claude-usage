...
**毎日 Claude Code を使っているのに「credentials stale」と表示される。**
*この*マシンの認証情報ファイルが更新されていません。よくある原因は2つ：別のマシン（SSH/WSL/コンテナ）で Claude Code を使っている、または CLI がサブスクリプションではなく `ANTHROPIC_API_KEY` で認証している — Claude Code 内で `/status` を確認し、`/login` でログインしてください。前者はローカルで一度 `claude` を実行すれば解決します。
